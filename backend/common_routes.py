...
def get_state_base(device=EasyMCP2221):
    ...
    for name, address in audio_slaves.items():
        ...
        # update te all device status 
        alldevices_state = False  if ('OFF' in message.values())  else True
    return message, alldevices_state, ivm_state
...
def sweep_devices():
    ...
    if device:
        ...
        if adresses :
            ...
            return jsonify({'success':{'Deviceses' : DEVICE.I2C,'vbso':vbso,'vbias':vbias,'state': get_state_base(device=device)}}),200
    else:
